 ...
 */
function softme_sanitize_checkbox( $checked ) {
	// Boolean check.
 ...
 */
function softme_sanitize_html( $html ) {
 ...
 */
function softme_sanitize_select( $input, $setting ) {
 ...
 */
function softme_sanitize_url( $url ) {
	return esc_url_raw( $url );
}


/* Sanitization Text*/
function softme_sanitize_text( $text ) {
	return wp_filter_post_kses( $text );
}

/* Sanitization Integer*/
function softme_sanitize_integer( $input ) {
    if( is_numeric( $input ) ) {
        return intval( $input );
    }
}